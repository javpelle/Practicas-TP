...
/**
 * A factory for creating Ataxx games. See {@link DraughtsRules} for the
 * description of the game.
 * 
 * 
 * <p>
 * Factoria para la creacion de juegos Ataxx. Vease {@link DraughtsRules}
 * para la descripcion del juego.
 */
public class DraughtsFactory implements GameFactory {

	private int dim;
	private int obstacles;

	public DraughtsFactory() {
		this(7, 0);
	}

	public DraughtsFactory(int dim, int obstacles) {
		if (dim < 5 || dim%2 == 0) {
	...
	public GameRules gameRules() {
		return new DraughtsRules(dim, obstacles);
	}

	@Override
	public Player createConsolePlayer() {
		ArrayList<GameMove> possibleMoves = new ArrayList<GameMove>();
		possibleMoves.add(new DraughtsMove());
		return new ConsolePlayer(new Scanner(System.in), possibleMoves);
	}

	@Override
	public Player createRandomPlayer() {
		return new DraughtsRandomPlayer();
	}
