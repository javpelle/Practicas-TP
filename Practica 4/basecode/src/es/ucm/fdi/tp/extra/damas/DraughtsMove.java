 ...
 * 
 */
public class DraughtsMove extends GameMove {
	 ...
	 * {@link DraughtsMove} to generate game moves from strings by calling
	 * {@link #fromString(String)}
	 * 
	 * <p>
	 * Solo se debe usar este constructor para obtener objetos de
	 * {@link DraughtsMove} para generar movimientos a partir de strings usando
	 ...
	 */
	public DraughtsMove(int x, int y, int row, int col, Piece p) {
		super(p);
		this.row = row;
		this.col = col;
		this.rowOrigin = x;
	...
	public void execute(Board board, List<Piece> pieces) {
		if (board.getPosition(row, col) == null && distanciaLegal() && board.getPosition(rowOrigin, colOrigin) == getPiece()) {
	...
	protected GameMove createMove(int origRow, int origCol, int row, int col, Piece p) {
		return new DraughtsMove(origRow, origCol, row, col, p);
	}
