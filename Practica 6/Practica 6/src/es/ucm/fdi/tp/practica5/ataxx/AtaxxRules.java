...
public class AtaxxRules implements GameRules {
	...
	public double evaluate(Board board, List<Piece> playersPieces, Piece turn,Piece p) {
		double defenseCriterion = 0;
		double piecesOfP = board.getPieceCount(p);
		double totalPieces = piecesOfP;
		double piecesNoP = 0;
		
		for (Piece q: playersPieces) {
			if(!q.equals(p)){
				piecesNoP += board.getPieceCount(q);	
			}
		}
		totalPieces += piecesNoP;
		defenseCriterion = maxDamage(board,p);
		
		
		return (1.0 - defenseCriterion/4.0) * 0.7 + ((piecesOfP / totalPieces) - (piecesNoP / totalPieces)) * 0.3;
	}
	...
	private int maxDamage (Board board, Piece p) {
		int maxDamage = 0;
		
		for(int i = 0; i < board.getRows() && maxDamage != 8; i++) {
			for(int j = 0; j < board.getCols() && maxDamage != 8; j++) {
				if( board.getPosition(i, j) == null ){
					int x = comprobarVecinasHueco(board, p, j, i); 
					if (x > maxDamage && canBeAttacked(board,p,j,i)) {
						maxDamage = x;
					}
				}
			}
	...
	private int comprobarVecinasHueco(Board board, Piece p, int col, int row) {
		int contador = 0;
		int deltas[][] = {
			{-1,-1}, {-1,0}, {-1,1},
			{0,-1},	{0,1},
		...
		for (int[] c: deltas)  {
			if(esValida(row + c[0], col + c[1], board) && board.getPosition(row + c[0], col + c[1]) != null && board.getPosition(row + c[0], col + c[1]).equals(p)) {
				contador++;
			}
		}
		return contador;
	...
	private boolean canBeAttacked (Board board, Piece p, int col, int row) {
		...
		for (int[] c: deltas)  {
			if(esValida( c[0] + row, c[1] + col, board) && board.getPosition(c[0] + row, c[1] + col) != null &&
				 !board.getPosition(c[0] + row, c[1] + col).equals(p) &&  !board.getPosition(c[0] + row, c[1] + col).equals(obstacle)) {
					return true;
				}
			}
		return false;
